...
class personnel extends IndexController {
    ...
    function userinfo() {
        ...
        if ($admin['shopid'] == 1) {
            $con = '1 = 1';
            $role = $m_role->findAll('', '', 'id,name');
            $shop = $m_shop->findAll('', '', 'id,shopname');
            foreach ($shop as $k => $v) {
                $shops[$v['id']] = $v['shopname'];
            }
            $department = $m_department->findAll('', 'sort asc');
            foreach ($department as $k => $v) {
                $dresults[$v['id']] = $shops[$v['pid']] . '-' . $v['department'];
            }
            $position = $m_position->findAll(array('departmentid' => $department[0]['id']));
        } else {
            $con = 'shopid = ' . $admin['shopid'];
            $role = $m_role->findAll('shopid = ' . $admin['shopid'], '', 'id,name');
            $shop = $m_shop->findAll('', '', 'id,shopname');
            foreach ($shop as $k => $v) {
                $shops[$v['id']] = $v['shopname'];
            }
            $department = $m_department->findAll('pid = ' . $admin['shopid'], 'sort asc');
            foreach ($department as $k => $v) {
                $dresults[$v['id']] = $shops[$v['pid']] . '-' . $v['department'];
            }
            $position = $m_position->findAll(array('departmentid' => $department[0]['id']));
...
//        }
    ...
    function punchLst() {
        ...
        if ($name) {
            $con .= ' and (departmentname like "%' . $name . '%" or name like "%' . $name . '%")';
            $page_con['name'] = $name;
        }
        $sql = 'select a.*,b.departmentname,b.name from yld_kqdkjl as a left outer join yld_admin as b on a.uid = b.id where (' . $con . ') order by dkdt desc';
    ...
    function punchTotel() {
        ...
        foreach ($results as $k => $v) {
            ...
            $jb = spClass('m_kqdkjb')->findCount('uid = ' . $v['id'] . ' and type = 1 and dkdt like "' . $month . '%"');
            $r = array();
    ...
    function kqinfo(){
        ...
        $name = htmlspecialchars($this->spArgs('name'));
        if(!empty($name)){
            $where .= ' uname like "%'.$name.'%" or  udeptname like "%'.$name.'%"';
            $page_con['name'] = $name;
        }
