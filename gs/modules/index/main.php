            ...
            if (!empty($username) && !empty($password)) {
                ...
                if ($result['password'] == md5(md5($password))) {
                    if (empty($result['login'])) {
                        $data_admin['login'] = md5(md5($result['password'] . $result['id']));
                        $model_admin->update(array('id' => $result['id']), $data_admin);
                        $result['login'] = $data_admin['login'];
                    }
                    ...
                    $data['remark'] = '登录成功';
                    //set cookie TODO
                    setcookie('token', $result['login'], time() + 3600 * 24 * 1, '/');
                    spClass('m_login')->create($data);
                    //get token
                    $this->msg_json(1, '登录成功', array('token' => $result['login']));
                } else {
                    $data['name'] = $result['name'];
...
//         $results = spClass('m_customer')->findAll($con);
        $results = spClass('m_custmang')->findAll($con);
        
        foreach ($results as $k => $v) {
//             $results[$k]['name'] = $v['company'];
            $results[$k]['name'] = $v['custcname'];
        }
        $this->msg_json(1, '获取成功', $results);
    }
