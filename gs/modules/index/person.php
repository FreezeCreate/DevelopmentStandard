...
class person extends IndexController {
    ...
    function infor() {
        ...
        $type = (int) htmlentities($this->spArgs('type'));
        $name = urldecode(htmlspecialchars($this->spArgs('name')));
        $con = 'a.del = 0 and a.cid = '.$admin['id'];
        if($name){
        ...
        $sql = 'select a.*,b.isread,b.readdt from yld_infor as a left outer join yld_flow_todos as b on (a.id = b.tid and b.modelid = 1 and b.uid = '.$admin['id'].') where (' . $con . ') order by a.adddt desc';
