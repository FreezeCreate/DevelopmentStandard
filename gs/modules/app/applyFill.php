...
//             'recompany'   => '退货公司',
        ...
        $data = $this->receiveData($arg);
        if (!is_numeric($data['remoney'])) $this->returnError('退货金额必须是数字');
...
//                     $v['status']    = 2;
    ...
    {
        $admin   = $this->islogin();
//         $model   = spClass('m_invoice');    //purchase_caigou
...
//             'phone'      => '联系方式',
        ...
        $data = $this->receiveData($arg);
        if (!is_numeric($data['totalmoney'])) $this->returnError('购货总金额必须是数字');
        ...
        if($id){
            ...
            $data['cid']       = $admin['cid'];
            $data['status']    = 1;
            $data['supplier']  = $data['buldcom'];
            $up = $model->create($data);
            //副表数据新增
            if ($up){
                //采过程检验的新增处理
                spClass('m_qualitylog')->create(array(
                    'oid'     => $data['oid'],
                    'cid'     => $up,
                    'optid'   => $data['optid'],
                ...
                foreach ($_REQUEST['list'] as $k => $v){
                    ...
                    $v['cid']       = $admin['cid'];
                    $v['invoice_id']= $up;
                    $v['format']    = $v['goods_unit'];
//                     $v['supplier']  = $v['buldid'];
                    $v['supplier']  = $data['buldid'];
                    $v['num']       = $v['goods_num'];
                    $v['name']      = $v['goods_name'];
                    $v['pid']       = $up;
                    $v['status']    = 1;
                    if (!is_numeric($v['goods_price'])) $this->returnError('单价必须是数字');
                    if (!is_numeric($v['goods_num'])) $this->returnError('数量必须是数字');
...
//                         $o_data['order_explain'] = $goods_name['order_explain'];
        ...
        if($up){
            $this->sendUpcoming($admin, 7, $up, '【'.$data['billnum'].'】采购单');
            
            $this->sendMsgNotice($admin, 7, $up, '【'.$data['billnum'].'】采购单', 1);
            $this->returnSuccess('成功');
        }
        $this->returnError('失败');
    }
