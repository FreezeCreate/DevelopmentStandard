...
<html>
    ...
    <body>
        ...
        <div class="ContentBox">
            <div class="Tables">
                ...
                <form action="<?php echo spUrl($c, $a) ?>" method="get">
                    <label class="form-group">
                        物料用品：<select class="FrameGroupInput" name="gid">
			...
			<a class="btn btn-primary" onclick="fill_apply(26)"><div class="TablesAddBtn">＋添 加</div> </a>
			</div>
                ...
                <tbody>
                    ...
                    <tr class="Results<?php echo $v['id']?>">
                            ...
                            <td class="colorGre">
                                      ...
                                      <ul class="menu">
                                         <li class="menu-item">
                                        <a onclick="check_apply(26, <?php echo $v['id'] ?>)">详情</a></li>
                                        <?php if ($admin['id'] == $v['uid']) { ?>
                                          <li class="menu-item">  <a onclick="fill_apply(26, <?php echo $v['id'] ?>)">编辑</a></li>
                                            <?php if ($v['status'] != 0) { ?>
  ...
  <?php require_once TPL_DIR . '/layout/apply.php'; ?>
    <script type="text/javascript">
        // function voidBox(id) {
            // if (confirm('确定作废？')) {
                // $.get("<?php echo spUrl($c, "voidOfficeapl"); ?>", {id: id}, function(data) {
                    // if (data.status == 1) {
        ...
        // function del(id) {
            // if (confirm('确定删除？')) {
                // $.get("<?php echo spUrl($c, "delOfficeapl"); ?>", {id: id}, function(data) {
                    // if (data.status == 1) {
                        // $('.Results' + id).remove();
                        // table_sort();
                    // }
        ...
        // }
		 ...
		 function voidBox( id) {
			Confirm('确定作废？', function(e) {
				if (e) {
					$.post("<?php echo spUrl($c, "voidOfficeapl"); ?>",{id: id}, function(data) {
						if (data.status == 1) {
                        $('.Results' + id + ' .data-status').removeClass('color-green,color-red,color-gray');
                        $('.Results' + id + ' .data-status').addClass('color-gray');
                        $('.Results' + id + ' .data-status').text('已作废');
		 ...
		 function del( id) {
			Confirm('确定删除吗？', function(e) {
				if (e) {
					$.post("<?php echo spUrl($c, "delOfficeapl"); ?>",{id: id}, function(data) {
						if (data.status == 1) {
							$('.Results' + id).remove();
