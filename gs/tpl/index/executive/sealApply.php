				 ...
				 <div class="TablesSerch">
				     <form action="<?php echo spUrl($c, $a) ?>" method="get">
						<label class="form-group">
							印章：<select class=" TablesSerchInput" name="sealid">
								<option value="0">全部</option>
								<?php foreach ($seals as $v) { ?>
									<option <?php echo $page_con['sealid'] == $v['id'] ? 'selected=""' : '' ?> value="<?php echo $v['id'] ?>"><?php echo $v['name'] ?></option>
								<?php } ?>
							</select>
						</label>
						<label class="form-group">
							<input type="text" class="TablesSerchInput" name="uname" value="<?php echo $page_con['uname']?>" placeholder="申请人"/>
						</label>
					...
					<a class="btn btn-primary" onclick="fill_apply(5)"><div class="TablesAddBtn">＋ 添 加 </div> </a>
				</div>
            ...
            <table class="table table-info table-hover sealapplication">
                ...
                <tbody>
                    ...
                    <tr class="Results<?php echo $v['id']?>">
                            ...
                            <td><?php echo $v['sealname'] ?></td>
                            <td><?php echo $v['isout'] == 1 ? '是' : '否' ?></td>
                            ...
                            <td class="colorGre">
                                            ...
                                            <ul class="menu">
            								 ...
            								 <li class="menu-item"><a onclick="check_apply(5, <?php echo $v['id'] ?>)">详情</a></li>
            								<?php if ($admin['id'] == $v['uid']) { ?>
            							   <li class="menu-item"> <a onclick="fill_apply(5, <?php echo $v['id'] ?>)">编辑</a></li>
            								<?php if ($v['status'] != 0) { ?>
    ...
    <script type="text/javascript">
        ...
        // }
			...
			Confirm('确定删除该信息吗？', function(e) {
				if (e) {
					$.post("<?php echo spUrl($c, "delSealapl"); ?>",{id: id}, function(data) {
						if (data.status == 1) {
							$('.Results' + id).remove();
			 ...
			 function voidBox( id) {
			Confirm('确定作废该信息吗？', function(e) {
				if (e) {
					$.post("<?php echo spUrl($c, "voidSealapl"); ?>",{id: id}, function(data) {
						if (data.status == 1) {
							$('.Results' + id).remove();
							Alert(data.msg, function() {
								window.location.reload();
