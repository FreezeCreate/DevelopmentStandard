            ...
            <div class="add-right" style="padding-top:10px; width:980px;">
                ...
                <table class="reg-table" style="width:900px; float:left;" cellpadding="0" cellspacing="0">
                    <tr>
                        ...
                        <td class="td-2">
                             <div class="inp-w">
                                <input type="text" name="phone1"value="<?php echo isset($result["phone1"])?$result["phone1"]:''; ?>" class="txt phone"  />
                             </div>
                        </td>
                        <td class="td-3">&nbsp;</td>
                    </tr>
                    <tr>
                        ...
                        <td class="td-2">
                             <div class="inp-w">
                                <input type="text" name="phone2"  value="<?php echo isset($result["phone2"])?$result["phone2"]:''; ?>" class="txt phone" />
                             </div>
                        </td>
                        <td class="td-3">&nbsp;</td>
                    </tr>
                    <tr>
                        ...
                        <td class="td-2">
                            <input type="text" placeholder="请输入品牌名" class="txt"  id="brand" /> &nbsp;&nbsp;<a href="Javascript:void(0)" onclick="brand_sub()">搜索</a>&nbsp;&nbsp;<a href="Javascript:void(0)" onclick="$('#add_brand').css('display','block')">添加品牌</a>
...
<script>
    $(function(){
        $('.phone').blur(function(){
            ...
            $.ajax({
                cache: false,
                type: "POST",
                url: "<?php echo spUrl("employees","phones"); ?>", //把表单数据发送到ajax.jsp
                data: {phone:phone}, //要发送的是ajaxFrm表单中的数据
                dataType: "json",
                async: false,
    ...
    function do_submit(){
        $.ajax({
            cache: false,
            type: "POST",
            url: "<?php echo spUrl("employees","save_card"); ?>", //把表单数据发送到ajax.jsp
            ...
            success: function(data) {  
            	...
            	if(data.status==1){
	                  window.location.href="<?php echo spUrl('employees','set_card');?>";
            	}
            }
        });
        return false;
