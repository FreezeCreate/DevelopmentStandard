	...
	*/
    ...
    InitDate.prototype = {
        init:function() {
            ...
            elem = opts.dateCell ? QD(opts.dateCell) :target;
            ...
            $(document).on('click', opts.dateCell, function(e){
            	jeDt.on(this, 'click', function(ev) {
                    jeDt.stopmp(ev);
                    that.setHtml(opts, this);
                });
            })
...
//                      jeDt.stopmp(ev);
//                      if (cel !== jeDt.elem) that.setHtml(opts, cel);
//                  });
//              });
            }
        },
        ...
        orien:function(obj, self, pos) {
			...
			var w	 = 0;
			var h	 = 0;
			if( ( box.width() + that.offset().left ) > $(window).width() ){
				w = that.offset().left-(box.width()-that[0].offsetWidth);
			}else{
				w = that.offset().left;
			};
			
			if( ( box.height() + that.offset().top+that[0].offsetHeight ) > $(window).height() ){
				h = that.offset().top-box.height();
			}else{
				h = that.offset().top+that[0].offsetHeight;
			}
			h < 45 ? h = 45 : '' ;
			box.css({left: w, top: h})
        },
