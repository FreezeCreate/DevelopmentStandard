...
//    		route_path = 'app.php/';
//    	}
        var html='<li><a class="MenuItem active on NewHtml a_01"data-url="home.html" data-clas="a_01" data-img="images/shouye_39.png" data-name="首页"data-url="home.html"><i class="firstImg home_1"></i><span class="firstText">首页</span></a></li><li class="l_loading"></li>';
        $("#LeftMenuBox").html(html);
        $.ajax({
            type:"get",
            data:{oid: oid,token:dataToken},
            ...
            success:function(data){
                ...
                var html='<li><a class="MenuItem active on NewHtml a_01" data-url="home.html" data-clas="a_01" data-img="images/shouye_39.png" data-name="首页" data-url="home.html"><i class="firstImg home_1"></i><span class="firstText">首页</span></a></li>';
                var res=data.results,url='';
                if(!res)return false;
                for(var k in res){//{id,title,control,way,children,img}
                    if(res[k].id==366)continue;
                    var children=res[k].children;
                    var childHtml='',len=children.length;
                    url=len>0?res[k].control:(res[k].control+'/'+res[k].way);
                    if(len>0){
                        for(var i=0;i<len;i++){
							if(oid==0){
                                ...
                                '<span class="firstText">'+children[i].title+'</span></a>';
							}else{
                        ...
                        html+='<li class="MenuGroup">'+
                                '<div class="GroupFirst">'+
                                    '<i class="firstImg home_'+res[k].img+'"></i>'+
                                    '<span class="firstText">'+res[k].title+'</span>'+
                                '</div>'+
                                '<div class="GroupFirstBox">'+childHtml+'</div>'+
                            '</li>';
                    }else{						
                        html+='<li>'+
                            '<a class="MenuItem NewHtml" data-url="'+url+'.html" data-clas="a_'+res[k].id+'" data-name="'+res[k].title+'">'+
                                '<i class="firstImg home_'+res[k].img+'"></i>'+
                                '<span class="firstText">'+res[k].title+'</span>'+
                            '</a>'+
                        '</li>';
    ...
    showCommonList('/app.php/main/todoNotice',function(data){$('#note').html(data.num)});
    showCommonList('/app.php/main/userInfo',function(data){
        var res=data.results;
        var html='';
        ...
        if(res){
            ...
            html='<tr>'+
                ...
                '<tr>'+
                    ...
                    '<td><input class="FrameGroupInput" type="text" name="phone" value="'+res.phone+'" /></td>'+
                    '<td class="FrameGroupName">短号 ：</td>'+
                    '<td><input class="FrameGroupInput" type="text" name="trumpet" value="'+res.trumpet+'" /></td>'+
                '</tr>'+
                '<tr>'+
                    '<td class="FrameGroupName">省份证号 ：</td>'+
                    '<td><input class="FrameGroupInput" type="text" name="idCard" value="'+res.idCard+'" /></td>'+
                    '<td class="FrameGroupName">生日 ：</td>'+
                    '<td><input class="FrameDatGroup" type="text" name="birthday" id="birth" placeholder="点击选择时间" value="'+res.birthday+'" /></td>'+
                '</tr>'+
                '<tr>'+
                    '<td class="FrameGroupName">邮箱 ：</td>'+
                    '<td><input class="FrameGroupInput" type="text" name="email" value="'+res.email+'" /></td>'+
                    '<td class="FrameGroupName">QQ ：</td>'+
                    '<td><input class="FrameGroupInput" type="text" name="QQ" value="'+res.QQ+'" /></td>'+
                '</tr>';
