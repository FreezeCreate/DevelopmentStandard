...
//         if (!strpos($_SERVER['HTTP_REFERER'], 'token')) $this->msg_json('非法调用');
     ...
     * 发送消息提醒入库
     */
    function sendMsgNotice($mid, $tid, $summary, $type = 1, $did = '')
    {
        ...
        if ($type == 2){
            if (!empty($did)){
                if (strpos($did, ',')){ //多部门
                    $arr_did = explode(',', $did);
                    foreach ($arr_did as $k => $v){
                        $user_data1 = spClass('m_admin')->findAll(array('did' => $v, 'cid' => $admin['cid']), '', 'id');
                        if (empty($user_data1)) continue;
                        
                        $user_data = array_merge($user_data, $user_data1);
                    }
                }else { //单部门
                    $user_data = spClass('m_admin')->findAll(array('did' => $did, 'cid' => $admin['cid']), '', 'id');
                }
            }else { //所有部门
                $user_data = spClass('m_admin')->findAll(array('cid' => $admin['cid']), '', 'id');
            }
            //save
            foreach ($user_data as $ak => $av){
                $data_all = array(
                    'modelid'   => $mid,
                ...
                $model->create($data_all);
                @$this->jpushSendMsg($data_all['modelname'], $summary, $av['id']);
            }
            //jpush的两端提醒处理
//             $this->jpushSendMsg($model_data['modelname'], $summary);
        }else {
            $m_set      = spClass('m_flow_set');
            $model      = spClass('m_flow_todos');
            $model_data = $m_set->find('id='.$mid.'');
