//var ImgUrl = 'http://192.168.1.195:5500/',
...
function getPagesBox({nowUrl,page,distance,addList}){
    ...
    $.ajax({
        type:"get",
        url:dataURL+nowUrl,
        data:{token:dataToken,page,searchname,distance},
        dataType:"json",
        success(data){
            $(".noMsg").hide();
            $(".l_loading").hide();
            ...
            $(".TablesHeadNav").on('click','.TablesHeadItem',function(){
                let that=$(this);
...
function addDataList(url){
    $("#addDataSub").click(function(){
        ...
        if(addData){
            if(addData.invoice_id)addData.invoice_name=filtDataId(addData.invoice_id,'invoice_id');
            if(addData.buldid)addData.buldcom=filtDataId(addData.buldid,'buldid');
            if(addData.recomid)addData.recompany=filtDataId(addData.recomid,'recomid');
            if(addData.addid)addData.addname=filtDataId(addData.addid,'addid');
            if(addData.apply_id)addData.apply_name=filtDataId(addData.apply_id,'apply_id');
            if(addData.salesid)addData.salesname=filtDataId(addData.salesid,'salesid');
            if(addData.custumid)addData.custname=filtDataId(addData.custumid,'custumid');
            if(addData.stock_id)addData.stock_name=filtDataId(addData.stock_id,'stock_id');
            if(addData.goods_id)addData.goods_name=filtDataId(addData.goods_id,'goods_id');
            if(addData.inven_house_id)addData.inven_house_name=filtDataId(addData.inven_house_id,'inven_house_id');
            if(addData.inven_num)addData.inven_name=filtDataId(addData.inven_num,'inven_num');
            if(addData.device_cateid)addData.device_catename=filtDataId(addData.device_cateid,'device_cateid');
            if(addData.deviceid)addData.devicename=filtDataId(addData.deviceid,'deviceid');
            if(addData.devicecateid)addData.devicecatename=filtDataId(addData.devicecateid,'devicecateid');
            if(addData.checkid)addData.checkname=filtDataId(addData.checkid,'checkid');
            if(addData.paytypeid)addData.paytype=filtDataId(addData.paytypeid,'paytypeid');
            if(addData.paydid)addData.paydname=filtDataId(addData.paydid,'paydid');
            if(addData.did)addData.dname=filtDataId(addData.did,'did');
            if(addData.depid)addData.depname=filtDataId(addData.depid,'depid');
            if(addData.sealid)addData.sealname=filtDataId(addData.sealid,'sealid');
            if(addData.cateid)addData.catename=filtDataId(addData.cateid,'cateid');
        }
...
function addCustRecord(url){
    $("#addCustSub").click(function(){
        ...
        if(addSubData.paytypeid)addSubData.paytype=filtDataId(addSubData.paytypeid,'paytypeid');
        if(addSubData.contractid)addSubData.contractid=$(".contractname").attr("id");
        if(addSubData.contractname)addSubData.contractname=$(".contractname").html();
        if(addSubData.paycname)addSubData.paycname=$(".paycname").html();
