 ...
 */
class passport extends IndexController {
    ...
    function logout() {
        unset($_SESSION['user']);
        $this->jump(spUrl('passport', 'login'));
    }
    ...
    function regCompany() {
        ...
        $data_u['login'] = md5(time() . $data['username']);
        $data_u['cid'] = $cid;
        $data_u['did'] = $did;
        $data_u['dname'] = '管理员';
        $data_u['pid'] = '1';
    ...
    function regPerson() {
        ...
        $data['idcard'] = Common::base64_image_content($data['idcard'], 'logo/' . date('Ymd'));
        $data['status'] = 1;
        $data['login'] = md5(time() . $data['username']);
    ...
    function bind() {
        $user = $this->islogin();
        $i = htmlentities($this->spArgs('i'));
        $apply = spClass('m_reg_company')->find('is_read = 0 and del = 0 and uid = ' . $user['id'], 'applydt desc', 'cname,dname,pname,applydt,status,`explain`,checkdt');
        $apply['applydt'] = date('Y-m-d',  strtotime($apply['applydt']));
        $apply['checkdt'] = date('Y-m-d',  strtotime($apply['checkdt']));
        if($apply){
            $this->apply = $apply;
            $this->i = $i==1?0:$apply['status'];
        }else if($user['cid']>0){
